...
@click.argument("--print", '-p', is_flag=True)
@click.argument("--show", "-s")
def analysis(show, print, compare, compare_choice):
    if compare:
        if show == False and print == False:
            print("Nothing will happen unless you supply the command with either a -p (print) or -s (show) tag") 
        else:   
            logic.compare_municipalities_confirmed_cases(show=show, print_file=print)
    elif compare_choice:
